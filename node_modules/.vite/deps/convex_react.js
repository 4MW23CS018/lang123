...
import { _ as toByteArray, c as version, d as convexToJson, f as jsonToConvex, g as fromByteArray, h as validateDeploymentUrl, i as makeFunctionReference, l as ConvexError, m as parseArgs, r as getFunctionName } from "./api-lKZT7YM-.js";
...
function useQuery(queryOrOptions, ...args) {
	...
	if (isObjectOptions) {
		...
		if (queryOrOptions.args !== "skip") argsObject = parseArgs(queryOrOptions.args);
	} else {
	...
	const result = useQueries((0, import_react.useMemo)(() => skip || !queryReference ? {} : { query: {
...
function usePaginatedQuery(queryOrOptions, args, options) {
	...
	const throwOnError = isObjectOptions ? queryOrOptions.throwOnError ?? false : true;
	const { user: positionalResult } = usePaginatedQueryInternal(query, queryArgs, isObjectOptions ? { initialNumItems: queryOrOptions.initialNumItems } : options, throwOnError);
	if (!isObjectOptions) return positionalResult;
	return reshapeToObjectForm(positionalResult);
}
